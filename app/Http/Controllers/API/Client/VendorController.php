...
class VendorController extends Controller
{
    ...
    public function getVendorItemsForAI($id): JsonResponse
    {
        $restorant = Restaurant::where(['id' => $id, 'active' => 1])->with(['categories.items.variants.extras'])->first();
        $items = [];
        if ($restorant) {
            if ($restorant->categories) {
                foreach ($restorant->categories as $category) {
                    ...
                    foreach ($category->items as $item) {
                        // Create simplified item object with only necessary fields
                        $itemObj = [
                            'id' => $item->id,
                            'name' => mb_convert_encoding($item->name, 'UTF-8', 'UTF-8'),
                            'description' => strlen($item->description) > 100 ? 
                                mb_convert_encoding(substr($item->description, 0, 97) . '...', 'UTF-8', 'UTF-8') : 
                                mb_convert_encoding($item->description, 'UTF-8', 'UTF-8'),
                            'price' => $item->price,
                            'category' => mb_convert_encoding($category->name, 'UTF-8', 'UTF-8'),
                        ...
                        if ($item->extras && count($item->extras) > 0) {
                            ...
                            foreach ($item->extras as $extra) {
                                $itemObj['extras'][] = [
                                    'name' => mb_convert_encoding($extra->name, 'UTF-8', 'UTF-8'),
                                    'price' => $extra->price
                                ];
                            }
                        }
                        ...
                        if ($item->has_variants && count($item->variants) > 0) {
                            $itemObj['variants'] = [];
                            foreach ($item->variants as $variant) {
                                $itemObj['variants'][] = [
                                    'name' => mb_convert_encoding($variant->optionsList, 'UTF-8', 'UTF-8'),
                                    'price' => $variant->price
                                ];
                            }
                        }

                        $categoryItems[] = $itemObj;
                    }

                    if (count($categoryItems) > 0) {
                        $items[mb_convert_encoding($category->name, 'UTF-8', 'UTF-8')] = $categoryItems;
                    }
                }

                return response()->json([
                    'restaurant_name' => mb_convert_encoding($restorant->name, 'UTF-8', 'UTF-8'),
                    'restaurant_description' => mb_convert_encoding($restorant->description, 'UTF-8', 'UTF-8'),
                    'menu' => $items,
                    'status' => true,
                    'errMsg' => '',
