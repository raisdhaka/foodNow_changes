...
class CartController extends Controller
{
    ...
    public function getContent(): JsonResponse
    {
       ...
       //Suggestions
       $suggestions = $this->getSuggestions($cartItems);
    ...
    private function getSuggestions($cartItems){
        $suggestions = [];

        try{
            //Get all items for this restorant
            $restorantID = null;
        ...
        foreach($cartItems as $cartItem){
            $usedItems[] = $cartItem->name;
            $usedItemsIDs[] = $cartItem->attributes->id;
        }

        //Get all items for this restorant
        $items = Items::whereHas('category', function($query) use ($restorantID) {
            $query->where('company_id', $restorantID);
        })
        ->whereNotIn('id', $usedItemsIDs)
        ->get();
        $randomItem = $items->random();

        //If openai api key is not set, return empty array
        if($restaurant->getConfig('openai_api_key', '') == '' || strlen($restaurant->getConfig('openai_api_key', '')) <= 3){
            //Get random item as suggestion if no OpenAI key
           
            return [
                'name' => $randomItem->name,
                'id' => $randomItem->id,
                'image' => $randomItem->icon,
        ...
        foreach( $items as $item){
            $suggestions[] = ['name' => $item->name, 'id' => $item->id];
        }

        //Call OpenAI API
        $openai = Http::withHeaders([
            'Authorization' => 'Bearer ' . $restaurant->getConfig('openai_api_key'),
        ...
        $response = $openai->post('chat/completions', [
            ...
            'messages' => [
                [
                    'role' => 'system',
                    'content' => 'You are a restaurant waiter. Your goal is to suggest menu items based on what the customer has already ordered. Return only a single item in JSON format with these fields: name, id. Look in all the options available. Ex with Pizza you can suggest a coke or a dessert. If there is already a dessert in the cart, you can suggest a coffee. If there is already something, do not suggest similar items. Ex, if there is a salad, do not suggest a salad again.'
                ],
                [
                    'role' => 'user', 
                    'content' => 'I have these items in my cart: ' . implode(', ', $usedItems) . '. From these available items, suggest one item to add next. Available items: ' . json_encode($suggestions) . '. Return only a single suggestion in JSON format.'
                ]
            ],
            'temperature' => 0.5,
            'max_tokens' => 800
        ...
        if(isset($response['choices'][0]['message']['content'])) {
            ...
            Log::info('Suggestion: ' . json_encode($suggestion));
            if($suggestion && isset($suggestion['name']) && isset($suggestion['id'])) {
                //Load the item
                Log::info('Loading item: ' . $suggestion['id']);
                $item = Items::where('id', $suggestion['id'])->first();
                return [
                    'name' => $item->name,
                    'id' => $item->id, 
                    'image' => $item->icon,
