...
class FrontEndController extends Controller
{
    ...
    public function rproMode()
    {
        if (config('settings.disable_landing')) {
            ...
            $response = new \Illuminate\Http\Response(view('landing', [
                'col' => $colCounter[count($plans)],
                'plans' => $plans,
                'faqs' => $faqs,
    ...
    public function restorant($alias)
    {
        ...
        try{
            if (!Cart::getContent()->isEmpty() && Cart::getContent()->first() && Cart::getContent()->first()->attributes && Cart::getContent()->first()->attributes->restorant_id != $restorant->id) {
                Cart::clear();
            }
        }catch(\Exception $e){
        }
        ...
        if ($restorant && $restorant->active == 1) {
            ...
            if ($menuTemplate != 'defaulttemplate') {
                if($menuTemplate == 'clean'){
                    $viewFile = 'restorants.show';
                }else{
                    $viewFile = config('settings.front_end_template', 'glow').'::show';
                }
            }
            ...
            // Check if the current restaurant has a webreswidgets record
            $webResWidget = null;
            try {
                $webResWidget = \DB::table('webreswidgets')
                    ->where('company_id', $restorant->id)
                    ->first();
                
                if ($webResWidget) {
                    // Store the webreswidgets ID in the view data
                    $viewData['webResWidgetId'] = $webResWidget->id;
                }
            }catch(\Exception $e){
                //Do nothing
            }
            ...
            //If we have photos module, get the albums and pass them to the view
            if (Module::has('photos')) {
                $viewData['hasPhotos'] = true;
                $viewData['albums'] = \Modules\Photos\Models\Album::where('company_id', $restorant->id)->with('photos')->orderBy('sort_order')->get();
            }else{
                $viewData['hasPhotos'] = false;
            }
