...
trait HasComplexDelivery
{
    ...
    public function setAddressAndApplyDeliveryFee()
    {
        ...
        try {
            ...
            $url = 'https://maps.googleapis.com/maps/api/distancematrix/json?departure_time=now&destinations='.$this->order->delivery_lat.','.$this->order->delivery_lng.'&origins='.$this->order->pickup_lat.','.$this->order->pickup_lng.'&key='.config('settings.google_maps_api_key_server', config('settings.google_maps_api_key', ''));
