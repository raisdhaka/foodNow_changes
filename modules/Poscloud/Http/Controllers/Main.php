...
use App\Models\User;
use Modules\Poscloud\Entities\AiOrder;
...
class Main extends Controller
{
    ...
    /**
     * Create a new AI order from API request
     * @param Request $request
     * @return Response
     */
    public function createAiOrder(Request $request)
    {
        try {
            $request->validate([
                ...
                'is_pickup' => 'sometimes|string',
                'is_deliver' => 'sometimes|string',
            ...
            if (!$user->restaurant) {
                return response()->json(['error' => 'User has no associated restaurant'], 400);
            }
            ...
            $aiOrder = AiOrder::create([
                ...
                'type' => $request->is_deliver.""=="true" ? 1 : 2, // 1 for pickup, 2 for delivery
                'time' => $request->time,
                'delivery_address' => $request->is_deliver.""=="true" ? $request->user_address : null,
                'company_id' => $user->restaurant->id,
                'processed' => false,
            ]);
    ...
    /**
     * Get all AI orders for the current restaurant
     * @return Response
     */
    public function getAiOrders()
    {
        ...
        try {
            ...
            $aiOrders = AiOrder::where('company_id', auth()->user()->restaurant->id)
                ->orderBy('created_at', 'desc')
                ->get();

            return response()->json([
                'status' => true,
                'count' => $aiOrders->count(),
                'orders' => $aiOrders
            ]);
        } catch (\Exception $e) {
            return response()->json([
     ...
     * Process an AI order
     * 
     * @param int $id The ID of the AI order to process
     * @return Response
     */
    public function processAiOrder($id)
    {
        try {
            if (!auth()->user()->restaurant) {
                return response()->json(['error' => 'User has no associated restaurant'], 400);
            ...
            $aiOrder = AiOrder::where('id', $id)
                ->where('company_id', auth()->user()->restaurant->id)
                ->first();

            if (!$aiOrder) {
                return response()->json([
                    'status' => false,
                    'message' => 'AI order not found'
                ], 404);
            }

            // Mark the order as processed
            $aiOrder->processed = true;
            $aiOrder->save();

            return response()->json([
                'status' => true,
                'message' => 'AI order processed successfully',
                'order' => $aiOrder
            ]);
        } catch (\Exception $e) {
            return response()->json([
