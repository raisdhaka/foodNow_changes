...
function updatePrices(net,delivery,enableDelivery){
  ...
  cartTotal.suggestion_price_formated=formatter.format(cartTotal.suggestion.price);
...
function getCartContentAndTotalPrice(){
   axios.get('/cart-getContent').then(function (response) {
    ...
    cartTotal.suggestion=response.data.suggestions;
    cartTotal.suggestion_price_formated="";
  ...
  cartTotal= new Vue({
    ...
    methods: {
      openSuggestion: function () {
       return setCurrentItemInGlow(cartTotal.suggestion.id);
      },
    },
    computed: {
