...
function getAllAiOrders(){
  axios.get('/api/v2/posapi/ai-orders?api_token=' + API_TOKEN)
    .then(function (response) {
      if (response.data.status) {
        aiOrders.aiItems = response.data.orders;
        aiOrdersTotal.totalAiOrders=response.data.count;
      }
    })
    .catch(function (error) {
      console.log(error);
...
window.onload = function () {
  ...
  // Add new AI Orders Vue instance
  aiOrders = new Vue({
    el: '#aiOrders',
    data: {
      aiItems: []
    ...
    methods: {
      processAiOrder: function(item) {
        ...
        console.log(item);
               ...
               // Set up the order based on type
              if(item.type == 1) { // Delivery
                
                createDeliveryOrder();

                setTimeout(function(){
                  $('#client_name').val(item.customer_name);
                  $('#client_phone').val(item.customer_phone);
                  if(item.delivery_address) {
                    $('#client_address').val(item.delivery_address);
                  }
                },1000);
              } else { // Takeaway
              ...
              //Set the ai note
            setTimeout(function(){
              $('#aiNoteContainer').show();
              $('#aiNoteText').val(item.content + '\n\nTime: ' + item.time);
            },1000);
        
       

        // Mark the order as processed
        axios.post('/api/v2/posapi/ai-orders/' + item.id + '/process?api_token=' + API_TOKEN)
          .then(function (response) {
            if (response.data.status) {
              // Show success message
              js.notify('AI Order processed successfully', 'success');
              
              // Refresh AI orders list
              getAllAiOrders();
          ...
          .catch(function (error) {
            console.log(error);
            js.notify('Error processing AI order', 'warning');
          });
      }
    }
  })
